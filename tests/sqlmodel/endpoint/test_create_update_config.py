"""
Tests for CreateConfig, UpdateConfig, and DeleteConfig functionality (SQLModel).
...
async def test_update_with_auto_fields(
    ...
    assert fetched_record.tier_id == 2
...
async def test_create_with_exclude_from_schema(
    ...
    assert fetched_record.tier_id == 5
...
async def test_update_with_exclude_from_schema(
    ...
    assert fetched_record.tier_id == 10
...
async def test_delete_with_auto_fields(
    ...
    assert fetched_record is not None
    assert fetched_record.tier_id == 99
    assert fetched_record.is_deleted is True
...
async def test_delete_config_multiple_auto_fields(
    ...
    delete_config = DeleteConfig(
        auto_fields={
            "tier_id": lambda: 999,
            "deleted_at": lambda: fixed_timestamp,
        }
    )
    ...
    assert fetched_record is not None
    assert fetched_record.tier_id == 999
    assert fetched_record.deleted_at == fixed_timestamp
    assert fetched_record.is_deleted is True
