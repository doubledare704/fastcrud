"""
Integration tests for Query Builder + Filter Processor with SQLModel.

This module tests how the filtering and query building engines work together
to create complete SQL queries with SQLModel models.
"""
...
from tests.sqlmodel.conftest import ModelTest
...
class TestQueryIntegration:
    ...
    def test_joined_filters_with_query_building(self):
        """Test joined filters integration with query building"""
        filter_processor = FilterProcessor(ModelTest)
        query_builder = SQLQueryBuilder(ModelTest)

        # Joined filters
        filters = filter_processor.parse_filters(**{"tier.name": "Premium"})
