...
from tests.sqlmodel.conftest import (
    Project,
    Participant,
...
async def test_many_to_many_nested_aggregates_and_counts_by_base(async_session):
    ...
    # Verify Project 1's participants (should have both Participant 1 and 2)
    proj1_participant_ids = {p["id"] for p in proj1["participant"]}
    proj1_participant_names = {p["name"] for p in proj1["participant"]}
    proj1_participant_roles = {p["role"] for p in proj1["participant"]}

    assert proj1_participant_ids == {1, 2}
    assert proj1_participant_names == {"Participant 1", "Participant 2"}
    assert proj1_participant_roles == {"Developer", "Designer"}

    # Verify Project 2's participant (should only have Participant 1)
    proj2_participant = proj2["participant"][0]
    assert proj2_participant["id"] == 1
    assert proj2_participant["name"] == "Participant 1"
    assert proj2_participant["role"] == "Developer"
